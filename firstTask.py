...
expName = 'firstTask'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\joann\\Downloads\\PsychoPy Presentation\\PsychoPy Presentation\\firstTask.py',
...
instructions1 = visual.TextStim(win=win, name='instructions1',
    text='Hello! Welcome to the task :)',
    font='Open Sans',
    pos=(0, 0), height=0.12, wrapWidth=1.3, ori=0.0, 
...
# Initialize components for Routine "regCues"
regCuesClock = core.Clock()
regCuesText = visual.TextStim(win=win, name='regCuesText',
    text='',
    font='Open Sans',
...
text_2 = visual.TextStim(win=win, name='text_2',
    text='How much joy do you feel?',
    font='Open Sans',
    pos=(0, 0.25), height=0.1, wrapWidth=1.3, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-1.0);
...
InstructionsComponents = [instructions1, key_resp]
...
trials = data.TrialHandler(nReps=1.0, method='random', 
    ...
    trialList=data.importConditions('regConditions.xlsx'),
...
for thisTrial in trials:
    ...
    # ------Prepare to start Routine "regCues"-------
    continueRoutine = True
    routineTimer.add(4.000000)
    # update component parameters for each repeat
    regCuesText.setText(RegulationCues)
    # keep track of which components have finished
    regCuesComponents = [regCuesText]
    for thisComponent in regCuesComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    regCuesClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "regCues"-------
    while continueRoutine and routineTimer.getTime() > 0:
        # get current time
        t = regCuesClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=regCuesClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        
        # *regCuesText* updates
        if regCuesText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            regCuesText.frameNStart = frameN  # exact frame index
            regCuesText.tStart = t  # local t and not account for scr refresh
            regCuesText.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(regCuesText, 'tStartRefresh')  # time at next scr refresh
            regCuesText.setAutoDraw(True)
        if regCuesText.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > regCuesText.tStartRefresh + 4.0-frameTolerance:
                # keep track of stop time/frame for later
                regCuesText.tStop = t  # not accounting for scr refresh
                regCuesText.frameNStop = frameN  # exact frame index
                win.timeOnFlip(regCuesText, 'tStopRefresh')  # time at next scr refresh
                regCuesText.setAutoDraw(False)
    ...
    # -------Ending Routine "regCues"-------
    for thisComponent in regCuesComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addData('regCuesText.started', regCuesText.tStartRefresh)
    trials.addData('regCuesText.stopped', regCuesText.tStopRefresh)
    ...
    slider.reset()
    # keep track of which components have finished
    ratingsComponents = [slider, text_2]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if slider.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > slider.tStartRefresh + 5.0-frameTolerance:
                # keep track of stop time/frame for later
